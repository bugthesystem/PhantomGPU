...
//!
//! This module provides benchmarking functionality for ML models on different GPUs.
//! It works with both synthetic models and real models loaded via the model_loader module.
...
/// Unified model information that can come from either synthetic or real models
...
impl ModelInfo {
    ...
    /// Convert RealModelInfo to ModelInfo for benchmarking
    #[cfg(feature = "real-models")]
    pub fn from_real_model_info(real_info: &RealModelInfo) -> Self {
        Self {
            name: real_info.name.clone(),
            parameters_m: (real_info.parameter_count.unwrap_or(0) as f64) / 1_000_000.0,
            input_shape: real_info.input_shapes
                .get(0)
                .unwrap_or(&vec![3, 224, 224])
                .iter()
                .map(|&x| if x > 0 { x as usize } else { 1 })
                .collect(),
            estimated_flops_g: real_info.estimated_flops_g,
            memory_mb: real_info.estimated_memory_mb,
        }
    }
}
...
pub async fn benchmark_pretrained_model(
    ...
    let model_info = match model {
        ...
        crate::cli::PretrainedModel::OnnxResnet50 => {
            ...
            ModelInfo::from_real_model_info(&real_model_info)
        }
